...
func (t *transferTool) Execute(args map[string]any) (string, error) {
	...
	// turn regardless.
	//
	// Step out of the call. Same three steps as an operator barging in, for
	// the same reason: the carrier re-forks the audio when our socket closes,
	// so leaving means latching the call as handed over, stopping the fork at
	// the carrier, and only then closing.
	leaveCall(t.callID, t.client, "transferred to "+t.to)
