...
type liveRelay struct {
	callID string
	// listenOnly withholds the operator's voice from the caller. Barge sends
	// it; listening does not, which is what makes listening silent to the
	// person on the phone.
	listenOnly bool
...
func registerRelay(sessionID, callID string, listenOnly bool) *liveRelay {
	r := &liveRelay{callID: callID, listenOnly: listenOnly}
	relayMu.Lock()
	relays[sessionID] = r
	relayMu.Unlock()
	return r
...
func handleRelayMedia(w http.ResponseWriter, r *http.Request) {
	...
	relay.ws = ws
	relay.mu.Unlock()
	log.Printf("relay: listen-in sidecar connected for session=%s (listen_only=%t)",
		sessionID, relay.listenOnly)
	...
	// Operator -> caller. Read until the sidecar leaves.
	for {
		...
		if relay.listenOnly || len(data) < 2 {
			// Listening is silent to the caller by definition, so the
			// operator's audio is read and discarded rather than never sent —
			// draining the socket keeps the sidecar from blocking on a full
			// send buffer.
			continue
		}
		calls.sendToCaller(relay.callID, data, relayRate)
	}
}
